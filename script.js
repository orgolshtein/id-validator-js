...
function validate(){
    ...
    isNaN(Number(input))=== true ? document.querySelector("#validator").innerHTML = `<h3 style="color: #ad0c49;">יש להקליד מספרים בלבד</h3>`
    : (input.length === 9) && ((sum) % 10 !== 0) ? document.querySelector("#validator").innerHTML = `<h2 style="color: #ad0c49;">מספר הזהות ${zeroPad(input,9)} לא תקין</h2>`
    : (input.length === 9) && ((sum) % 10 === 0) ? document.querySelector("#validator").innerHTML = `<h2 style="color: #33BBC5;">מספר הזהות ${zeroPad(input,9)} תקין</h2>`
    : input.length === 0 ? document.querySelector("#validator").innerHTML = `<h3 style="color: #E19898;">הקלד ספרות כדי לקבל את ספרת הביקורת</h3>`
